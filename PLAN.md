...
| P1-G | Production Postgres deploy (docker-compose service + init SQL + role passwords) + fix dead pgedge_analytics MCP placeholder + hermes-data volume config-sync entrypoint fix | P1-A, P1-B | company-ops/docker-compose.yml, company-ops/scripts/entrypoint.sh, company-ops/hermes/config.yaml, company-ops/.env (secrets, gitignored), company-ops/.env.example, company-ops/sql/init/04-set-role-passwords.sh (new), company-ops/NAHAR-TODO.md | 1 | opencode | claimed | 2026-09-07: manager live-verification confirmed no production Postgres exists anywhere (only the ephemeral test harness) and pgedge_analytics is a dead placeholder (@pgedge/mcp-server confirmed 404 on npm). Dispatched to opencode/mimo-v2.5-free with @microsoft/postgres-mcp pre-researched as the replacement (confirmed real, actively maintained, Node 22+ compatible, image has Node v26.5.1). Not yet verified done. |
...
| P-MGR6 | Fix supergateway crash on second/reconnecting SSE client against `engineering` MCP endpoint | P-MGR5 | company-ops/scripts/engineering-entrypoint.sh (final `exec` line only -- do NOT touch `sync_repo()`/the repo-clone loop above it), company-ops/Dockerfile.engineering (only if chosen fix needs a build-time change), company-ops/docker-compose.yml (`engineering` service block only, only if the chosen fix needs a port/env change) | opencode | dispatching | Filed 2026-09-07 after a live test this session: `hermes mcp test engineering_manager` connection #1 succeeded (SSE session opened+closed cleanly), connection #2 crashed the whole container with an uncaught `Error: Already connected to a transport... at supergateway/dist/gateways/stdioToSse.js:58` -- confirmed directly via `docker compose logs engineering` and `docker ps` showing `Exited (1)`. Real stability bug in supergateway's stdio-to-SSE bridge reusing one MCP SDK Server instance across reconnects, not a config typo. See dispatch ticket text in session log for full investigation instructions (try pinning a newer supergateway release, or swapping to `mcp-proxy`, or checking if `ai-cli-mcp` supports native HTTP/SSE server mode directly). |
| P-MGR7 | Diagnose inconsistent repo-clone SSH timeouts (`app`/`company-os` intermittently time out while `website`/`hermees` succeed with the same key, same container) and harden `sync_repo()` accordingly | P-MGR6 | company-ops/scripts/engineering-entrypoint.sh (`sync_repo()` function and its WARN messages only -- do NOT touch the final `exec` line, that is P-MGR6's territory), company-ops/NAHAR-TODO.md | opencode | todo | Filed 2026-09-07, sequenced after P-MGR6 since both land in the same file (different regions) to avoid a working-tree collision. Live evidence this session: `docker compose logs engineering` showed `company-os` and `hermees` BOTH failing with `ssh: connect to host github.com port 22: Connection timed out` in the same boot where a prior board note (P-MGR2) recorded `hermees` cloning successfully and `company-os` failing with a clean `Repository not found` (auth-level, not network-level) -- i.e. the specific repos and even the failure *mode* (timeout vs. clean rejection) are NOT consistent run-to-run, which is the inconsistency to actually diagnose, not paper over. NAHAR-TODO.md's Group H already documents a known real access gap (deploy key needs push access added on all 5 bmh repos, read-only works for the 4 public ones, `company-os` is private and needs read+write) -- do not duplicate that finding, but do confirm via repeated direct `ssh -T git@github.com` / `git ls-remote` probes (a few tries each, paused between) whether the *timeout* failures specifically are transient network flakiness (add bounded retry+backoff to `sync_repo()`) or something else, and make `sync_repo()`'s WARN output clearly distinguish timeout vs. auth-rejection vs. repo-not-found so future boots are self-diagnosing. |
